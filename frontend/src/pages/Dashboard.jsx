...
function AuroraRing({ calories, goal }) {
  ...
  useEffect(() => {
    ...
    const pct = clamp(calories / goal, 0, 1);
    ...
    function draw() {
      ...
      for (let i = 0; i < segments; i++) {
        ...
        if (progress > pct) break;
        const a1 = startA + progress * Math.PI * 2;
        const a2 = startA + ((i + 1) / segments) * Math.PI * 2;
        const colorPos = (progress * 0.8 + t) % 1;
        const [r, g, b] = getAuroraColor(colorPos);
        const alpha = 0.4 + progress * 0.6;
        const isLeading = progress > pct - 0.06;
      ...
      grad.addColorStop(0, `rgba(${ir},${ig},${ib},0.05)`);
...
export default function Dashboard() {
  ...
  // Group meals by slot
  function getMealsForSlot(slotKey) {
    return log.filter(m => (m.meal_type || m.meal_time) === slotKey);
  }
  function getSlotCals(slotKey) {
    return getMealsForSlot(slotKey).reduce((s, m) => s + (m.calories||0), 0);
  }
      ...
      <div className="dash">
        ...
        <div style={{ padding:"52px 22px 0", marginBottom:28 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            <div>
              ...
              <div style={{ fontSize:30, fontWeight:900, color:D.t1, letterSpacing:-1 }}>
                {firstName} 👋
              </div>
            </div>
        ...
        <div style={{ padding:"0 22px", marginBottom:28 }}>
          {[
            ...
            const pct = clamp(m.val / m.goal * 100, 0, 100);
            return (
              <div key={m.lbl} style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                ...
                <div style={{ fontSize:11, fontWeight:700, color:m.color, width:34, textAlign:"right", fontFamily:"'DM Mono',monospace" }}>{rnd(m.val)}g</div>
              </div>
            );
          })}
        </div>
          ...
          {MEAL_SLOTS.map((slot, si) => {
            ...
            return (
              <div key={slot.key} className="fade-up" style={{ animationDelay:`${si*0.05}s`, marginBottom:6 }}>
                ...
                >
                  {/* Left accent for current slot */}
                  {isNow && (
                    <div style={{ position:"absolute", left:0, top:0, bottom:0, width:3, background:`linear-gradient(180deg,${D.purple},${D.blue})`, borderRadius:"0 2px 2px 0" }} />
                  )}

                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{
                      width:38, height:38, borderRadius:12,
                      background: hasMeals ? "rgba(167,139,250,0.08)" : isNow ? "rgba(167,139,250,0.06)" : "rgba(255,255,255,0.03)",
                    ...
                    <div>
                      <div style={{ fontSize:11, fontWeight:700, letterSpacing:0.5, textTransform:"uppercase", color: isNow ? D.purple : hasMeals ? "rgba(255,255,255,0.7)" : D.t3 }}>
                        {slot.label}
                        {isNow && <span style={{ fontSize:8, marginLeft:6, color:D.purple, letterSpacing:1 }}>NOW</span>}
                      </div>
                      <div style={{ fontSize:10, color: isNow ? "rgba(167,139,250,0.45)" : D.t3, marginTop:2 }}>
                        {hasMeals
                          ? meals.map(m => m.food_name).join(", ").slice(0, 28) + (meals.map(m=>m.food_name).join(", ").length > 28 ? "…" : "")
                          : isNow ? "Tap + to log now" : "Not logged yet"
                        }
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    {hasMeals && (
                      <div style={{ fontSize:17, fontWeight:900, color: hasMeals ? "rgba(255,255,255,0.6)" : D.t3, fontFamily:"'DM Mono',monospace", letterSpacing:-0.5 }}>
                        {rnd(slotCals)}
                      </div>
                    )}
                    <button
                      ...
                      style={{
                        ...
                        background: isNow ? D.purple : "rgba(255,255,255,0.06)",
                        color: isNow ? "#000" : D.t2,
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            {[
              ...
              <button
                ...
                onClick={() => {
                  if (a.to) nav(a.to);
                  else if (a.name === "Water") setGlasses(g => g < 8 ? g+1 : 0);
                }}
                style={{
            ...
            {Array.from({ length:8 }).map((_, i) => (
              <button
                ...
                style={{
                  ...
                  background: i < glasses ? "rgba(96,165,250,0.2)" : "rgba(255,255,255,0.03)",
                  borderWidth:1, borderStyle:"solid",
                  borderColor: i < glasses ? "rgba(96,165,250,0.3)" : "rgba(255,255,255,0.05)",
                  transition:"all 0.2s",
                  fontSize:13,
                }}
              >
